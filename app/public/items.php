...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'create') {
        ...
        if ($title === '') {
            ...
            $stmt = $pdo->prepare('INSERT INTO items (title, description, created_by) VALUES (?,?,?)');
    ...
    if ($action === 'update') {
        ...
        $title = trim((string) ($_POST['title'] ?? ''));
        $description = trim((string) ($_POST['description'] ?? ''));
        $stmt = $pdo->prepare('UPDATE items SET title=?, description=? WHERE id=? AND created_by=?');
    ...
    if ($action === 'delete') {
        $id = (int) ($_POST['id'] ?? 0);
        $stmt = $pdo->prepare('DELETE FROM items WHERE id=? AND created_by=?');
        $stmt->execute([$id, $uid]);
        header('Location: /items.php');
        exit;
    }
...
$list = $pdo->prepare('SELECT id, title, description, created_at FROM items WHERE created_by = ? ORDER BY id DESC');
$list->execute([$uid]);
$rows = $list->fetchAll();
$editRow = null;
if ($editId > 0) {
    $e = $pdo->prepare('SELECT id, title, description FROM items WHERE id=? AND created_by=?');
...
<body>
  <h1>Items (CRUD)</h1>
  <p><strong>Replica:</strong> <code><?= htmlspecialchars($hostname, ENT_QUOTES, 'UTF-8') ?></code></p>
