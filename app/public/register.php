...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($email === '' || !filter_var($email, FILTER_VALIDATE_EMAIL)) {
        ...
        try {
            $stmt = Database::pdoWrite()->prepare(
...
<body>
  ...
  <?php if ($lastEmail !== '' && $error === 'Email already registered.'): ?>
    <p>
      <form method="post" action="/forgot-password.php" style="display:inline">
        <input type="hidden" name="email" value="<?= htmlspecialchars($lastEmail, ENT_QUOTES, 'UTF-8') ?>">
        <button type="submit">Send password reset email</button>
      </form>
      <span> — one-time link valid for 30 minutes (check Mailpit).</span>
    </p>
  <?php endif; ?>
  <form method="post">
